...
class SoccerHandler:
    ...
    def __init__(self, use_live_api: bool = False):
        ...
        if self.use_live_api:
            try:
                ...
                logger.info("Soccer Handler initialized with live ESPN API")
            except Exception as e:
                logger.warning(f"Failed to initialize live API, using mock data: {e}")
                self.use_live_api = False
        else:
            logger.info("Soccer Handler initialized with mock data")
    ...
    def fetch_games_today(self) -> List[Dict]:
        ...
        if self.use_live_api and self.sports_data:
            try:
                games_data = self.sports_data.get_soccer_games()
                    ...
                    try:
                        ...
                        if len(competitors) >= 2:
                            ...
                            formatted_games.append({
                                'game_id': game.get('id', f'soccer_{len(formatted_games)}'),
                                'home': home_team,
                                'away': away_team,
                ...
                if formatted_games:
                    logger.info(f"Fetched {len(formatted_games)} live Soccer games from ESPN")
                    return formatted_games
                else:
                    logger.info("No live Soccer games found, using mock data")
